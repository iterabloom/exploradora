...
# EXPL_FORGE_BACKEND override forces it).  Sourced here so the callers'
...
#   Set FORGE_BACKEND ("github" or "forgejo").  The EXPL_FORGE_BACKEND
...
detect_forge_backend() {
	...
	if [[ -n "${EXPL_FORGE_BACKEND:-}" ]]; then
		FORGE_BACKEND="$EXPL_FORGE_BACKEND"
...
#   backend EXPL_GITHUB_TOKEN wins, else FORGEJO_TOKEN.  Sets FORGE_TOKEN.
# ------------------------------------------------------------------
resolve_forge_token() {
	if [[ "${FORGE_BACKEND:-forgejo}" == "github" ]]; then
		# GitHub maintainer tooling uses a dedicated local PAT (EXPL_GITHUB_TOKEN,
		...
		# still origin.
		FORGE_TOKEN="${EXPL_GITHUB_TOKEN:-${FORGEJO_TOKEN:-}}"
	else
